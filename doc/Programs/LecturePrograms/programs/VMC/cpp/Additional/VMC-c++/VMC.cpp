//VMC-implementation for the Helium atom in pure C++
...
int main() {
  ...
  int antMC = 100000000; //Antall MC-sykluser
  ...
  double delta = .05; //Lengde av Metropolis-hopp
  ...
  for (alpha = 1.4; alpha <= 2.51; alpha += .05) {
    ...
    kjoerMC(sum, kvadratsum, antMC, delta, idum, alpha, N, accepted); //Utfør MC-beregning
    /*
