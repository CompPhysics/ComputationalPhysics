 ...
 *
 * This is a "librarized" version, intended to be controlled by an external C++ program
 * or a python script.
 *
 *
 * Written by M.H.Jensen, librarization by Kyrre N. Sjobak
 */
...
pendelum::pendelum(double m, double l, double omega, double A,
		   double viscosity, double phi_0, double v_0,
		   int N, double t_end)
{
  /* Constructor, based on initialize_keyboard
   *
   * Input:
   * - m:         Mass of the pendulum
   * - l:         Length of the pendulum rod
   * - omega:     Omega of the external force
   * - A:         Amplitude of the external force
   * - viscosity: The value of the viscous drag constant
   * - phi_0:     Starting point of pendulum (radians)
   * - v_0:       Starting velocity (radians/time)
   * - N:         Number of time integration steps
   * - t_end:     Final time steps as a multiplum of pi
   */
...
{
  /*
   * Initialization directly from the keyboard
   *
   */
...
void pendelum::euler_cromer()
{
  ...
  y_h[0]=y[0];  //phi
  y_h[1]=y[1];  //v
  ofstream fout("euler_cromer.out");
  fout.setf(ios::scientific);
  fout.precision(20);
  for(i=1; i<=n; i++){
    derivatives(t_h,y_h,yout);
...
void pendelum::euler_richardson()
{
  ...
  y_h[0]=y[0];  //phi
  y_h[1]=y[1];  //v
  ofstream fout("euler_richardson.out");
  fout.setf(ios::scientific);
  fout.precision(20);
  for(i=1; i<=n; i++){
    derivatives(t_h,y_h,yout);
