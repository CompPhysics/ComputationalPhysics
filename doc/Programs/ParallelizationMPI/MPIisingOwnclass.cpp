...
void InitializeLattice(int, Matrix &, double&, double&);
// The metropolis algorithm including the loop over Monte Carlo cycles
void MetropolisSampling(int, int, double, Vector &);
// prints to file the results of the calculations  
void WriteResultstoFile(int, int, double, Vector);
...
int main(int argc, char* argv[])
{
  ...
  for (double Temperature = InitialTemp; Temperature <= FinalTemp; Temperature+=TempStep){
    Vector LocalExpectationValues(5);
    ...
    // Find total average
    Vector TotalExpectationValues(5); 
    for( int i =0; i < 5; i++){
...
void MetropolisSampling(int NSpins, int MonteCarloCycles, double Temperature, Vector &ExpectationValues)
{
...
void InitializeLattice(int NSpins, Matrix &SpinMatrix,  double& Energy, double& MagneticMoment)
{
...
void WriteResultstoFile(int NSpins, int MonteCarloCycles, double temperature, Vector ExpectationValues)
{
