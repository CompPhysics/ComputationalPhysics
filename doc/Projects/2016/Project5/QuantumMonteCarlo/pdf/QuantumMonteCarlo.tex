...
applications to solar cells and nano-medicine. Although most studies are dne for electrons in two dimensions,
...
The relevant background material can be fund in chapter 14 of the lecture notes.
...
\]
Use this expression when developing your program. Find then the analytical expression for the second trial wave function
\[ 
E_{L2} = E_{L1}+?
\]
...
Compute also the expectation value of the mean distance at the energy minimun 
  ...
  $\omega=0.05$ and $\omega=1.0$.  Comment your results. 
...
With the optimal parameters for the ground state wave function, compute the expectation values of the  energy and the variance using $\omega=0.01$,
  $\omega=0.05$ and $\omega=1.0$. How important are the correlations introduced by the Jastrow factor? Comment your results. Compare the best VMC results wth those from project 2 for the same frequencies.  Compute also the expectation value of the mean distance at the energy minimun 
  $r_{12}=\sqrt{\mathbf{r}_1-\mathbf{r}_2}$ between the two electrons for the optimal set of the variational parameters using $\omega=0.01$,
  $\omega=0.05$ and $\omega=1.0$.  Comment your results and compare the results with and without the Jastrow factor.
