...
The aim of this project is to develop a program that uses Jacobi's (also called Givens') method for finding eigenvalues. We can start with the simple case of a buckling beam (or a quantum mechanical particle moving in a potential with infinite walls) and then move to  
Schroedinger's equation for two
electrons in a three-dimensional harmonic oscillator well with and
without a repulsive Coulomb interaction.  The eigenvalue solver you will end up with is however rather general and can be applied to different cases.   An important aspect in being able to achieve this is to scale properly the equationsand by reformulating the equations  in a discretized form as an eigenvalue
equation to be solved with Jacobi's method. To achieve this you will
have to write your own code which implements Jacobi's method.   Here we will specialize to a quantum mechanical system, but for those of you less familiar with quantum mechanics you can choose to focus more on the mathematical details of the problem.


What follows first is a longer description of the eigenvalue problem, with a specialization to the case of electrons confined to move in a potential well of the type of a harmonic oscillator.   This type of problem leads to the field of so-called quantum dots. 
